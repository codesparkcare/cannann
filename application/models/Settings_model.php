    ...
    private function ensure_opening_columns() {
        if ($this->db->table_exists('site_settings')) {
            $fields_to_check = array(
                ...
                'opening_date'        => "ALTER TABLE `site_settings` ADD `opening_date` DATETIME NULL DEFAULT '2026-09-12 09:00:00'",
                'opening_mode'        => "ALTER TABLE `site_settings` ADD `opening_mode` VARCHAR(50) NOT NULL DEFAULT 'countdown_page'",
                'opening_title'       => "ALTER TABLE `site_settings` ADD `opening_title` VARCHAR(255) NOT NULL DEFAULT 'Grand Opening — September 12, 2026'",
                'opening_subtitle'    => "ALTER TABLE `site_settings` ADD `opening_subtitle` TEXT NULL",
                'opening_banner_text' => "ALTER TABLE `site_settings` ADD `opening_banner_text` VARCHAR(255) NOT NULL DEFAULT '🎉 Grand Opening on September 12, 2026 — Pre-Bookings Now Open!'"
            );
